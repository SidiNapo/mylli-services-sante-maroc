...
const AccordionItem = ({ title, content, isOpen, onClick }: AccordionItemProps) => {
  return (
    <div className="border-b border-gray-200">
      <button
        className="flex w-full justify-between py-4 text-left font-medium text-gray-900 focus:outline-none"
        onClick={onClick}
      >
        <span>{title}</span>
        <span className="ml-6 flex items-center">
          {isOpen ? (
            <svg
              className="h-6 w-6 text-mylli-blue"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              aria-hidden="true"
            ...
            <svg
              className="h-6 w-6 text-mylli-darkblue"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              aria-hidden="true"
    ...
    <section className="bg-gray-50 py-16">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          <div>
            <img 
              src="/lovable-uploads/143f36ce-9b05-4bd3-8eee-c0ddd2ee8a66.png"
              alt="Soignant avec patient" 
