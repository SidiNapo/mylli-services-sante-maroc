...
const Testimonials = () => {
  ...
  return <section className="bg-gradient-to-r from-mylli-blue to-mylli-darkblue py-16">
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-4xl mx-auto relative">
          ...
          <div className="flex justify-center mt-6 space-x-2">
            {testimonials.map((_, index) => <button key={index} onClick={() => setActiveIndex(index)} className={`w-3 h-3 rounded-full ${index === activeIndex ? "bg-white" : "bg-blue-300"}`} aria-label={`View testimonial ${index + 1}`} />)}
          </div>

          <button onClick={prevTestimonial} className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 md:-translate-x-8 bg-white p-2 rounded-full shadow-md hover:bg-gray-100" aria-label="Previous testimonial">
            <svg className="w-6 h-6 text-mylli-darkblue" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
            </svg>
          </button>

          <button onClick={nextTestimonial} className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 md:translate-x-8 bg-white p-2 rounded-full shadow-md hover:bg-gray-100" aria-label="Next testimonial">
            <svg className="w-6 h-6 text-mylli-darkblue" fill="none" stroke="currentColor" viewBox="0 0 24 24">
